...
function parseSelectQuery(query) {
  try {
    ...
    const selectRegex =
      /^SELECT\s(.*?)\sFROM\s(.*?)(?:\s(INNER|LEFT|RIGHT)\sJOIN\s(.*?)\sON\s(.*?))?(?:\sWHERE\s(.*?))?(?:\sGROUP\sBY\s(.*?))?(?:\sORDER\sBY\s(.*?))?(?:\sLIMIT\s(\d+))?$/i;
    ...
    const groupByFields = matches[7]
      ? matches[7].split(',').map((field) => field.trim())
      : null;
    const orderByFields = matches[8]
      ? matches[8].split(',').map((field) => {
          const [fieldName, order] = field.trim().split(/\s+/);
          return { fieldName, order: order ? order.toUpperCase() : 'ASC' };
        })
      : null;
    const hasAggregateWithoutGroupBy =
      hasAggregateWithoutGroupBy_(fields) && !groupByFields;
